...
{
 __shared__ double shrA[BLOCKSIZE][BLOCKSIZE]; //safe
 __shared__ double shrB[BLOCKSIZE][BLOCKSIZE]; //safe

 size_t myRow = blockIdx.y*blockDim.y+threadIdx.y;
 size_t myCol = blockIdx.x*blockDim.x+threadIdx.x;
 
 size_t localRow=threadIdx.y;//local w.r.t. blocks
 size_t localCol=threadIdx.x;

 double c = 0.0; //c is in the memory directly attached to the core
