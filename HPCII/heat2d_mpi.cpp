...
int main(int argc, char* argv[])
{
 ...
 size_t N0 = 5; //10 // 2^N0 + 1 elements per side
 ...
 while (g[0].L2NormDiff > tolerance)  // Multigrid solver start
 {
  ...
  mpi_applyJacobi(g, 0, downRelaxations, nums[0], nums[1], coords[0], coords[1], left, right, top, bottom);
...
void mpi_applyJacobi(gridLevel* g, size_t l, size_t relaxations, int xdim, int ydim, int xcoord, int ycoord, int left, int right, int top, int bottom)
{
 ...
 size_t iFrom = ( g[l].N / xdim ) * xcoord;
 size_t iTo = ( g[l].N / xdim ) * (xcoord+1);
 size_t jFrom = ( g[l].N / ydim ) * ycoord;
 size_t jTo = ( g[l].N / ydim ) * (ycoord+1);
 ...
 for (size_t r = 0; r < relaxations; r++)
 {
  ...
  for (size_t i = iFrom; i < iTo; i++)
   for (size_t j = jFrom; j < jTo; j++) // Perform a Jacobi Iteration
