...
#pragma unroll 1024
 ...
 { 
  double xDist = xPos[m] - x[idx];
  double yDist = yPos[m] - y[idx];
  double zDist = zPos[m] - z[idx];
  __syncthreads();

  double r     = sqrt(xDist*xDist + yDist*yDist + zDist*zDist);
  //double 1r =  1. / (r*r*r);
  xFor[m] += xDist*mass[m]*mass[idx] / (r*r*r);
  yFor[m] += yDist*mass[m]*mass[idx] / (r*r*r);
  zFor[m] += zDist*mass[m]*mass[idx] / (r*r*r);
 }
}
}
