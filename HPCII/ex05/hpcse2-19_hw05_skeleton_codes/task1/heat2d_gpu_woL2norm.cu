...
//kernel fct. Eex9Koh4pha-
__global__ void reduceKernel(double *g_idata, double *g_odata, size_t N) {
...
//kernel fct. Eex9Koh4pha-
__global__ void dummyReductionKernel(double *temp, size_t N, double *result)
{
double tmp =0.0;
...
//*/
/* for (size_t i = 0; i < N; i++){
 	cudaMemcpy(g[0].Res[i], &g[0].dRes[i*g[l].N], N*sizeof(double),cudaMemcpyDeviceToHost);	checkCUDAError("cpy Res To Host in L2");
 }
*/
double *temp; 
//cudaMalloc(&temp, sizeof(double));	checkCUDAError("Alloc error temp");
temp =  (double*) malloc(sizeof(double) * N * N);
cudaMemcpy(temp, g[l].dRes,  N *N*sizeof(double),cudaMemcpyDeviceToHost);	checkCUDAError("cpy Res To Host in L2");
cudaDeviceSynchronize();
/* 
int num_blocks = ceil(N*N/32);
double *dtmp;
cudaMalloc(&dtmp, sizeof(double)* N*N);checkCUDAError("Alloc error dtmp"); 
cudaDeviceSynchronize();
dtmp= g[l].dRes;
double *tmp;
cudaMalloc(&tmp, sizeof(double)* N*N);checkCUDAError("Alloc error tmp"); 
cudaDeviceSynchronize();
cudaMemset(tmp, 0.0, sizeof(double)* N*N);
cudaDeviceSynchronize();
while(num_blocks > 0){
 reduceKernel<<< num_blocks,32 >>>(dtmp, tmp, N*N);checkCUDAError("Calculate first reduction error"); 
 num_blocks =ceil(num_blocks/32);if (num_blocks==1)break;
 double* temp = dtmp;   dtmp = tmp;   tmp	 = temp;
}
//*/

//double *tmp2;
//cudaMalloc(&tmp2, sizeof(double));checkCUDAError("Alloc error tmp2"); 
//cudaMemset(tmp2, 0.0, sizeof(double));checkCUDAError("Memset to 0 tmp2"); 
//reduceKernel<<< 1 , 32 >>>(tmp, dtmp, N*N);checkCUDAError("Calculate 2nd reduction error");
//*/
/*
 for (size_t i = 0; i < g[l].N; i++)
...
//*
 for (size_t i = 0; i < g[l].N; i++)
  for (size_t j = 0; j < g[l].N; j++)
   tmp += temp[i*N+j];
//*/
 //dummyReductionKernel<<< 1 , 1 >>>(g[l].dRes , N, temp);
 //cudaDeviceSynchronize();checkCUDAError("error sync ");
 //double result = 0.0;
 //cudaMemcpy(&result, temp, sizeof(double), cudaMemcpyDeviceToHost);	checkCUDAError("error copying tmp");
 g[l].L2Norm = sqrt(tmp);
