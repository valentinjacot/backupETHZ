...
// cudaDeviceSynchronize();
// cudaMemcpy(g,&d_g,sizeof(gridLevel) * gridCount, cudaMemcpyDeviceToHost);checkCUDAError("memcpy 2 error");
 cudaDeviceSynchronize();
...
dim3 blocksPerGrid;
if(N<32) {threadsPerBlock =dim3(32,32);blocksPerGrid=dim3(ceil(N),ceil(N));}
else {threadsPerBlock=dim3(32,32);
blocksPerGrid=dim3(ceil(N/32),ceil(N/32));}//depends on the size of the problem. int or dim3
...
//*
  ...
  applyJacobiKernel<<< blocksPerGrid,threadsPerBlock >>>(g[l].N, h1, h2, g[l].dUn, g[l].df, g[l].dU);checkCUDAError("apply Jacobi error");
  cudaDeviceSynchronize();
//*/
/*
...
//*/
/*
 calculateResidualKernel<<< blocksPerGrid,threadsPerBlock >>>(d_g, l, h2);
*/
 auto t1 = std::chrono::system_clock::now();
...
//*/
/*
 applyRestrictionKernel<<< blocksPerGrid,threadsPerBlock >>>(g, l);
*/
 auto t1 = std::chrono::system_clock::now();
