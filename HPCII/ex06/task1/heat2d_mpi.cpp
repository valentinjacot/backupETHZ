...
*/
 ...
 {
  mpi_applyJacobi(g, 0, downRelaxations, dims[0], dims[1], coords[0], coords[1], right, left, up, down, gridComm); // Relaxing the finest grid first
...
/*void mpi_applyJacobi(gridLevel* g, size_t l, size_t relaxations, int xdim, int ydim, int xcoord, int ycoord,int right,
	int left,int up,int down, MPI_Comm gridComm)
{
 ...
 /*for (size_t i = 0; i < g[0].N; i++)
	memcpy(&lU[g[0].N*i],g[0].U[i],g[0].N*sizeof(double));
	//memcpy(lUn[g[0].N*i],g[0].Un[i],g[0].N*sizeof(double)); 
  ...
  if(jTo == g[l].N) jTo--;
	...
	MPI_Type_vector(nx, 1, g[l].N, MPI_DOUBLE, &faceXType); 
	MPI_Type_commit(&faceXType);
	MPI_Type_vector(1, ny, ny, MPI_DOUBLE, &faceYType); 
 ...
 for (size_t r = 0; r < relaxations; r++)
 {
  ...
  double* tmp = g[l].Un; g[l].Un = g[l].U; g[l].U = tmp;
   for (size_t i = iFrom; i <iTo; i++)
    ...
    g[l].U[i * N + j] = (g[l].Un[ (i-1) * N + j] + g[l].Un[ (i+1) * N + j] + g[l].Un[i * N + j-1] + g[l].Un[i * N + j+1] + g[l].f[i][j]*h2)*h1;
	
	MPI_Irecv(&g[l].U[0            ], 	1, faceXType, 	right, 	1, gridComm, &request[request_count++]);
    MPI_Irecv(&g[l].U[(nx+1)       ], 	1, faceXType, 	left, 	1, gridComm, &request[request_count++]);
    MPI_Irecv(&g[l].U[0            ], 	1, faceYType, 	up, 	1, gridComm, &request[request_count++]);
    MPI_Irecv(&g[l].U[(ny+1)		], 	1, faceYType,	down, 	1, gridComm, &request[request_count++]);
  
  
	MPI_Isend(&g[l].U[1            		], 1, faceXType,	 	right, 	1, gridComm, &request[request_count++]);
	MPI_Isend(&g[l].U[nx		      	], 1, faceXType, 		left, 	1, gridComm, &request[request_count++]);
	MPI_Isend(&g[l].U[nx+2  			], 1, faceYType, 		up, 	1, gridComm, &request[request_count++]);
	MPI_Isend(&g[l].U[(nx+2)*ny     	], 1, faceYType, 		down, 	1, gridComm, &request[request_count++]);
 
 }
/* for (size_t i = 0; i < g[0].N; i++)
	memcpy(g[0].U[i], &lU[g[0].N*i],g[0].N*sizeof(double));
	//memcpy(g[0].Un[i], lUn[g[0].N*i],g[0].N*sizeof(double));
 
 auto t1 = std::chrono::system_clock::now();
 smoothingTime[l] += std::chrono::duration<double>(t1-t0).count();
}
*/
void mpi_applyJacobi(gridLevel* g, size_t l, size_t relaxations, int xdim, int ydim, int xcoord, int ycoord, int left, 
	int right, int top, int bottom, MPI_Comm comm)
{
...
void mpi_calculateResidual(gridLevel* g, size_t l, int xdim, int ydim, int xcoord, int ycoord)
{
 ...
 for (size_t i = iFrom; i <iTo; i++)
  for (size_t j= jFrom; j < jTo; j++)
   g[l].Res[i][j] = g[l].f[i][j] + (g[l].U[(i-1)*N+j] + g[l].U[(i+1)*N+j] - 4*g[l].U[i*N+j] + g[l].U[i*N+j-1] + g[l].U[i*N+j+1]) * h2;
