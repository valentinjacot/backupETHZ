        ...
        const size_t bufsize, const size_t Nx, const size_t Ny, const double tol)
{
    ...
    fheader.Nx  = Nx;
    fheader.Ny  = Ny;
    fheader.Nb  = static_cast<size_t>(size); // each MPI process works with one block
    ...
    if (MPI_File_open(MPI_COMM_WORLD, fname.c_str(), MPI_MODE_CREATE | MPI_MODE_WRONLY | MPI_MODE_UNIQUE_OPEN, MPI_INFO_NULL, &fh))
    {
        cerr << "Can not create MPI file '" << fname << "'" << endl;
        exit(1);
    }

    // 2.)
    if (isroot)
        MPI_File_write_at(fh, 0, &fheader, base, MPI_CHAR, &st);
    MPI_File_write_at_all(fh, base + rank*sizeof(BlockHeader), &bheader, sizeof(BlockHeader), MPI_CHAR, &st);

    // 3.)
    MPI_File_write_at_all(fh, boffset, cbuf, cbytes, MPI_CHAR, &st);
        ...
        size_t& cbytes, size_t& bufsize, size_t& Nx, size_t& Ny, double& tol)
{
    ...
    if (MPI_File_open(MPI_COMM_WORLD, fname.c_str(), MPI_MODE_RDONLY | MPI_MODE_UNIQUE_OPEN, MPI_INFO_NULL, &fh))
    {
        cerr << "Can not create MPI file '" << fname << "'" << endl;
        exit(1);
    }
    ...
    if (fheader.Nb != static_cast<size_t>(size))
    {
        // the number of MPI ranks must match the number of blocks in the file
        // (our simplicity protocol from write_data)
        cerr << "Number of MPI processes is not the same as number of compressed blocks!" << endl;
        exit(1);
    }
    tol = fheader.tol;
    Nx  = fheader.Nx;
    ...
    MPI_File_read_at_all(fh, base + rank*sizeof(BlockHeader), &bheader, sizeof(BlockHeader), MPI_CHAR, &st);
